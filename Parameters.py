...
class Parameters(tk.Frame):  # Definition de notre class Parameters 
    ...
    #////////////////////////////// CONTENEUR FRAME POUR FPS et POUR RADIOBUTTON  /////////////////////////////////////////////////////    
        ...
        contchoix=tk.Frame(contFPS,width=150,height=100, borderwidth=2, relief=GROOVE,bg='ivory')
        
        self.cocher= StringVar()
        case_A = tk.Radiobutton(contchoix,text="sequentiel", variable=self.cocher, value=1)
        case_A.pack(pady=10,padx=10)
        case_B = tk.Radiobutton(contchoix,text="global", variable=self.cocher, value=2)
        case_B.pack(pady=10,padx=10) 
                  
        contchoix.pack(padx=10,pady=5,side="top")
        contchoix.pack_propagate(False)      
    ...
    def save(self):          #On récupère la valeur de fps, duration, nom de fichier text, et  On l'enregistre dans les variables
            ...
            minute=self.mm.get() 
            seconde=self.ss.get()
            choix=self.cocher.get()
            
             
            if  minute!="" and nomFile!="" and heure!="" and fps!="" and seconde!="" and choix!=0:      #si tous les info ont été saisi , message de confirmation et on peut envoyer les parametres             
                ...
                source.writerow([choix,file,fps,heure,minute,seconde,nomFile]) 
