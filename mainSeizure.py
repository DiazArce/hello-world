...
if __name__ == '__main__' :
   ...
   # variables de début de traitement de la vidéo -> arg[6] est l'index de l'heure, multiplié par 3600 secondes, multiplié par 1000 millisecondes
    ...
    tbegin=float()
    index_duration=int()
    source = csv.reader(open("fichier.csv", "r")) #à revoir pour la récuperation de l'index de l'heure ///////////////////////////////////////////////////
    for row in source:
            index_duration = row[2]
     
     
    tbegin = Index_duration * 3600 * 1000 
    source.close()
   ...
   #si on est en fin du fichier vidéo, et qu'il ne s'agit pas d'une heure complète, alors on met à jour tend en fonction
    ...
    fpsInfo = vidcap.get(cv2.CAP_PROP_FPS)
   
    if (nFrames / fpsInfo) * 1000 < tend:
        tend = vidcap.get(nFrames) / vidcap.get(fpsInfo)*1000 
